...
public:
  ...
  inline explicit EndTime(unsigned int millisecondsIntoTheFuture)
  ...
  {
    return totalWaitTime == InfiniteValue ? false
      : (totalWaitTime == 0 ? true : (SystemClockMillis() - startTime) >= totalWaitTime);
  }
